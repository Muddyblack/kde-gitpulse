...
sub diffstat {
    my (%a) = @_;
    my ($x, $y, $add, $del) = @a{qw(x y add del)};
    my $o = '';
    my $add_txt = "+$add";
    my $del_txt = "-$del";
    my $atw = length($add_txt) * 6.5;
    my $dtw = length($del_txt) * 6.5;
    $o .= text(x => $x, y => $y + 12.5, size => 10, weight => 600, mono => 1, color => $C{positive}, t => $add_txt);
    $o .= text(x => $x + $atw + 4, y => $y + 12.5, size => 10, weight => 600, mono => 1, color => $C{negative}, t => $del_txt);
    return ($o, $atw + 4 + $dtw);
}

sub section_header {
    my (%a) = @_;
    my ($x, $y, $w, $title, $hint) = @a{qw(x y w title hint)};
    my $o = '';
    $o .= text(x => $x, y => $y + 10, size => 11, weight => 600, color => $C{dim}, t => $title);
    my $tw = length($title) * 6.8 + 8;
    my $hw = $hint ? length($hint) * 6.0 + 8 : 0;
    my $line_start = $x + $tw;
    my $line_end = $x + $w - $hw;
    if ($line_end > $line_start) {
        $o .= sprintf('<line x1="%.1f" y1="%.1f" x2="%.1f" y2="%.1f" stroke="%s" stroke-width="1"/>',
            $line_start, $y + 6, $line_end, $y + 6, $C{line});
    }
    if ($hint) {
        $o .= text(x => $x + $w, y => $y + 10, size => 10, color => $C{faint}, anchor => 'end', t => $hint);
    }
    return $o;
}
...
sub commit_clock {
    my (%a) = @_;
    my ($cx, $cy, $hours, $peak, $tz) = @a{qw(cx cy hours peak tz)};
    my $o = '';
    my $outer    = 54;
    my $tickR    = 47;
    my $rim      = 38;
    ...
    # Rim: day arc (bottom semicircle 06:00 to 18:00 via 12:00)
    $o .= sprintf('<path d="M %.1f %.1f A %.1f %.1f 0 0 1 %.1f %.1f" fill="none" stroke="%s" stroke-width="1.5" stroke-opacity="0.5" stroke-linecap="round"/>',
        $cx + $rim, $cy, $rim, $rim, $cx - $rim, $cy, $C{accent});

    # Rim: night arc (top semicircle 18:00 to 06:00 via 00:00)
    $o .= sprintf('<path d="M %.1f %.1f A %.1f %.1f 0 0 1 %.1f %.1f" fill="none" stroke="%s" stroke-width="1.5" stroke-opacity="0.35" stroke-linecap="round"/>',
        $cx - $rim, $cy, $rim, $rim, $cx + $rim, $cy, $C{dim});

    # Current time hand (e.g. 21.2 hours)
    my $handAngle = (-90 + 21.2 * 15) * $pi / 180;
    $o .= sprintf('<line x1="%.1f" y1="%.1f" x2="%.1f" y2="%.1f" stroke="%s" stroke-opacity="0.25" stroke-width="1"/>',
        $cx, $cy, $cx + $rim * cos($handAngle), $cy + $rim * sin($handAngle), $C{accent});

    # Spokes (24 hours)
    my $span = $spokeMax - $hub - 2;
    for my $h (0 .. 23) {
        my $ang = (-90 + $h * 15) * $pi / 180;
        my $v = $hours->[$h] // 0;
        my $end = $hub + 2 + ($v / $peak) * $span;
        my $lit = ($h == 21); # peak hour
        my $col = $lit ? $C{accent} : ($v > 20 ? $C{accent} : $C{dim});
        my $op = $lit ? 1.0 : ($v > 0 ? 0.4 + 0.55 * ($v / $peak) : 0.25);
        my $sw = $lit ? 3.8 : 2.8;

        $o .= sprintf('<line x1="%.1f" y1="%.1f" x2="%.1f" y2="%.1f" stroke="%s" stroke-opacity="%.2f" stroke-width="%.1f" stroke-linecap="round"/>',
            $cx + $hub * cos($ang), $cy + $hub * sin($ang),
            $cx + $end * cos($ang), $cy + $end * sin($ang),
            $col, $op, $sw);
    }

    # Hub
    $o .= circle(cx => $cx, cy => $cy, r => 2.4, fill => $C{accent});

    # Hour tick marks: 00, 06, 12, 18
    $o .= text(x => $cx, y => $cy - $tickR + 2, size => 8.5, color => $C{faint}, anchor => 'middle', t => '00');
    $o .= text(x => $cx + $tickR + 3, y => $cy + 3.5, size => 8.5, color => $C{faint}, anchor => 'middle', t => '06');
    $o .= text(x => $cx, y => $cy + $tickR + 6, size => 8.5, color => $C{faint}, anchor => 'middle', t => '12');
    $o .= text(x => $cx - $tickR - 3, y => $cy + 3.5, size => 8.5, color => $C{faint}, anchor => 'middle', t => '18');

    # Caption
    $o .= text(x => $cx, y => $cy + $outer + 15, size => 9, mono => 1, color => $C{faint}, anchor => 'middle', t => "commits by hour · $tz");
...
sub streak_figure {
    my (%a) = @_;
    my ($cx, $cy, $current, $longest, $label, $sub) = @a{qw(cx cy current longest label sub)};
    my $o = '';
    my $r = 18;
    my $pi = 3.14159265358979;

    # Ring background track
    $o .= sprintf('<circle cx="%.1f" cy="%.1f" r="%.1f" fill="none" stroke="#ffffff" stroke-width="2.6" stroke-opacity="0.10"/>',
        $cx, $cy, $r);

    # Progress arc
    my $frac = $longest > 0 ? $current / $longest : 0;
    my $endAng = -90 + $frac * 360;
    my $rad = $endAng * $pi / 180;
    my $ex = $cx + $r * cos($rad);
    my $ey = $cy + $r * sin($rad);
    my $large = $frac > 0.5 ? 1 : 0;
    $o .= sprintf('<path d="M %.1f %.1f A %.1f %.1f 0 %d 1 %.1f %.1f" fill="none" stroke="%s" stroke-width="2.6" stroke-linecap="round"/>',
        $cx, $cy - $r, $r, $r, $large, $ex, $ey, $C{accent});

    # Value
    $o .= text(x => $cx, y => $cy + 5.5, size => 16, weight => 700, color => $C{accent}, anchor => 'middle', t => $current);
    # Label & sub
    $o .= text(x => $cx, y => $cy + 30, size => 10.5, color => $C{dim}, anchor => 'middle', t => $label);
    $o .= text(x => $cx, y => $cy + 43, size => 8.5, color => $C{faint}, anchor => 'middle', t => $sub);
...
sub stat_figure {
    my (%a) = @_;
    my ($cx, $cy, $val, $label, $sub, $accent) = @a{qw(cx cy val label sub accent)};
    my $o = '';
    my $col = $accent ? $C{accent} : $C{text};
    $o .= text(x => $cx, y => $cy + 5.5, size => 18, weight => 700, color => $col, anchor => 'middle', t => $val);
    $o .= text(x => $cx, y => $cy + 30, size => 10.5, color => $C{dim}, anchor => 'middle', t => $label);
    $o .= text(x => $cx, y => $cy + 43, size => 8.5, color => $C{faint}, anchor => 'middle', t => $sub);
    return $o;
}
...
sub vertical_rule {
    my ($x, $y1, $y2) = @_;
    return sprintf('<line x1="%.1f" y1="%.1f" x2="%.1f" y2="%.1f" stroke="%s" stroke-width="1"/>', $x, $y1, $x, $y2, $C{line});
}
...
sub row {
    ...
    my $mx = 58;
    if ($a{forge}) {
        $o .= text(x => $mx, y => $y + 35, size => 10, weight => 600, mono => 1, color => $C{dim}, t => $a{forge});
        $mx += length($a{forge}) * 6.2 + 3;
        $o .= circle(cx => $mx, cy => $y + 31.5, r => 1.5, fill => $C{faint});
        $mx += 6;
    }

    $o .= text(x => $mx, y => $y + 35, size => 11, mono => 1, color => $C{dim}, t => $a{meta});
    $mx += length($a{meta}) * 6.2 + 8;

    my ($p, $pw) = pill(x => $mx, y => $y + 24, t => $a{state}, color => $tone, glyph => $a{stateIcon});
    $o .= $p;
    $mx += $pw + 6;

    if (defined $a{add} && defined $a{del}) {
        my ($ds, $dsw) = diffstat(x => $mx, y => $y + 24, add => $a{add}, del => $a{del});
        $o .= $ds;
        $mx += $dsw + 6;
    }
...
## ── inbox ────────────────────────────────────────────────────────────────────
{
    ...
    $b .= row(y => 158, tone => $C{accent}, icon => 'pull', bold => 1,
        ...
        forge => 'GH', meta => 'muddyblack/gitpulse · #14',
        state => 'review', stateIcon => 'dot',
        add => 48, del => 12, age => '4m');
    $b .= row(y => 212, tone => $C{accent}, icon => 'pull', bold => 1,
        title => 'Review requested: drop the vendored copy',
        forge => 'CB', meta => 'muddyblack/forgejo-thing · #3',
        state => 'review', stateIcon => 'dot',
        add => 120, del => 4, age => '11m');
    $b .= row(y => 266, tone => $C{accent}, icon => 'at', bold => 1,
        title => '@muddyblack can you confirm the path?',
        forge => 'GL', meta => 'muddyblack/ai-usage · #42',
        state => 'mention', stateIcon => 'dot', age => '52m');
    $b .= divider(328, 'EARLIER');
    $b .= row(y => 336, tone => $C{faint}, icon => 'merge',
        title => 'PR merged: waybar cleanup',
        forge => 'GH', meta => 'muddyblack/dotfiles · #5',
        state => 'merged', stateIcon => 'check',
        add => 16, del => 8, age => '1d');
    $b .= footer($w, $h);
...
# ── actions ──────────────────────────────────────────────────────────────────
{
    ...
    $b .= row(y => 140, tone => $C{negative}, icon => 'cross', bold => 1,
        ...
        forge => 'GH', meta => 'muddyblack/nixos-config · main',
        state => 'failure', stateIcon => 'cross', age => '26m');
    $b .= row(y => 194, tone => $C{accent}, icon => 'refresh', bold => 1,
        title => 'build-plasmoid · package',
        forge => 'GL', meta => 'muddyblack/gitpulse · feat/tabs',
        state => 'running', stateIcon => 'refresh', age => '2m');
    ...
    my $bx = 28;
    for my $btn (['Open', 'ext', 1], ['Merge request #12', 'pull', 0], ['Re-run', 'refresh', 0], ['Repository', 'issue', 0]) {
...
# ── profile ──────────────────────────────────────────────────────────────────
{
    ...
    # Account switcher strip (when more than one account is configured)
    $b .= rect(x => 14, y => 106, w => 134, h => 22, rx => 11, fill => $C{accent});
    $b .= text(x => 81, y => 121, size => 10.5, weight => 700, color => $C{onaccent}, anchor => 'middle', t => 'GitHub · muddyblack');

    $b .= rect(x => 154.5, y => 106.5, w => 148, h => 21, rx => 10.5, stroke => $C{line});
    $b .= text(x => 228, y => 121, size => 10.5, color => $C{dim}, anchor => 'middle', t => 'Codeberg · muddyblack');

    # Identity block and counters
    $b .= avatar(id => 'profile-avatar', cx => 42, cy => 164, r => 28);
    $b .= text(x => 82, y => 152, size => 17, weight => 700, color => $C{text}, t => 'Muddyblack');
    $b .= text(x => 82, y => 168, size => 11, mono => 1, color => $C{accent}, t => '@muddyblack');
    $b .= text(x => 82, y => 183, size => 10.5, color => $C{dim}, t => 'information should be free');
    $b .= text(x => 82, y => 196, size => 9.5, color => $C{faint}, t => 'Germany · joined 10y ago');

    my @stats = (
        ['139', 'stars'], ['22', 'commits'], ['58', 'PRs'], ['23', 'issues'],
        ['12', 'reviews'], ['62', 'repos'], ['76', 'followers'], ['5', 'orgs'],
    );
    for my $i (0 .. $#stats) {
        my $col = $i % 4;
        my $row = int($i / 4);
        my $sx = 242 + $col * 47;
        my $sy = 152 + $row * 27;
        $b .= text(x => $sx, y => $sy, size => 12.5, weight => 700, color => $C{text}, t => $stats[$i][0]);
        $b .= text(x => $sx, y => $sy + 11, size => 8.5, color => $C{faint}, t => $stats[$i][1]);
    }

    # ── when I ship & ActivityBand ──────────────────────────────────────────
    $b .= section_header(x => 14, y => 212, w => 412, title => 'When I ship', hint => 'mostly evening');

    # 24-hour dial on left (cx=88, cy=290)
    my @dialHours = (2, 1, 0, 0, 1, 1, 2, 4, 8, 14, 18, 16, 15, 12, 16, 20, 24, 28, 38, 52, 60, 68, 48, 22);
    $b .= commit_clock(cx => 88, cy => 290, hours => \@dialHours, peak => 68, tz => 'utc+2');

    # Rule separating dial from figures
    $b .= vertical_rule(170, 244, 350);

    # Three figures on the right
    $b .= stat_figure(cx => 216, cy => 272, val => '1,293', label => 'Contributions', sub => 'Jan 4 — now', accent => 1);
    $b .= vertical_rule(258, 252, 340);
    $b .= streak_figure(cx => 300, cy => 272, current => 5, longest => 16, label => 'Current streak', sub => 'since Sep 1');
    $b .= vertical_rule(344, 252, 340);
    $b .= stat_figure(cx => 388, cy => 272, val => '16', label => 'Longest streak', sub => 'Apr 30 — May 15', accent => 0);

    # ── contributions & heatmap ─────────────────────────────────────────────
    $b .= section_header(x => 14, y => 386, w => 412, title => 'Contributions', hint => '271 active days');
    my $seed = 12345;
    ...
    for my $col (0 .. 51) {
        for my $r (0 .. 6) {
            ...
            $b .= rect(x => 14 + $col * ($cell + $gap), y => 398 + $r * ($cell + $gap),
                w => $cell, h => $cell, rx => 1.6, fill => $fill, opacity => $op);
        }
    }
    $b .= text(x => 14, y => 464, size => 9.5, color => $C{faint}, t => 'plus 650 in private repositories');

    # ── last 30 days & trend chart ──────────────────────────────────────────
    $b .= section_header(x => 14, y => 482, w => 412, title => 'Last 30 days', hint => 'peak 17 · avg 3.6/day');
    my @pts;
    $seed = 99;
    for my $i (0 .. 29) {
        $seed = ($seed * 1103515245 + 12345) % 2147483648;
        my $v = ($seed / 2147483648) ** 2;
        push @pts, sprintf('%.1f,%.1f', 14 + $i * (412 / 29), 530 - $v * 28);
    }
    $b .= qq{<polyline points="14,530 } . join(' ', @pts) . qq{ 426,530" fill="$C{accent}" fill-opacity="0.18"/>};
    $b .= qq{<polyline points="} . join(' ', @pts) . qq{" fill="none" stroke="$C{accent}" stroke-width="1.6" stroke-linejoin="round"/>};

    # ── languages ───────────────────────────────────────────────────────────
    $b .= section_header(x => 14, y => 546, w => 412, title => 'Languages', hint => 'by bytes, own repositories');
    my @languages = (['QML', 38, '#44a51c'], ['TypeScript', 24, '#3178c6'], ['JavaScript', 18, '#f1e05a'], ['Python', 12, '#3572a5'], ['Nix', 8, '#7e7eff']);
    my $lx = 14;
    for my $lang (@languages) {
        my $lw = 412 * $lang->[1] / 100;
        $b .= rect(x => $lx, y => 558, w => $lw, h => 8, fill => $lang->[2]);
        $lx += $lw;
    }
    $lx = 14;
    for my $lang (@languages) {
        $b .= circle(cx => $lx + 3, cy => 582, r => 3, fill => $lang->[2]);
        $b .= text(x => $lx + 10, y => 585, size => 9, color => $C{dim}, t => $lang->[0] . ' ' . $lang->[1] . '%');
...
# ── status ───────────────────────────────────────────────────────────────────
{
    ...
    $b .= rect(x => 14, y => 106, w => $w - 28, h => 36, rx => 8, fill => $C{neutral}, opacity => 0.12);
    $b .= rect(x => 14.5, y => 106.5, w => $w - 29, h => 35, rx => 8, stroke => 'rgba(210,153,34,0.35)');
    $b .= circle(cx => 32, cy => 124, r => 5, fill => $C{neutral});
    $b .= text(x => 46, y => 123, size => 13, weight => 700, color => $C{text}, t => 'Partially degraded service');
    $b .= text(x => 46, y => 136, size => 10, color => $C{dim}, t => 'Monitoring 6 services');

    $b .= rect(x => 14, y => 152, w => 104, h => 22, rx => 11, fill => $C{accent});
    $b .= text(x => 66, y => 167, size => 11, color => $C{onaccent}, anchor => 'middle', t => 'Components 6');
    $b .= rect(x => 124.5, y => 152.5, w => 88, h => 21, rx => 10.5, stroke => $C{line});
    $b .= text(x => 168, y => 167, size => 11, color => $C{dim}, anchor => 'middle', t => 'Incidents 1');

    $b .= section_header(x => 14, y => 184, w => 412, title => 'Components', hint => '90 days · incident-free');

    my @svcs = (
        ['Git Operations', 0.01, 'operational', $C{positive}],
        ['Actions', 0.14, 'degraded performance', $C{neutral}],
        ['API Requests', 0.08, 'operational', $C{positive}],
        ['Pull Requests', 0.04, 'operational', $C{positive}],
        ['Webhooks', 0.05, 'operational', $C{positive}],
        ['Copilot', 0.07, 'operational', $C{positive}],
    );
    my $sy   = 202;
    my $seed = 4242;
    for my $s (@svcs) {
        $b .= circle(cx => 18, cy => $sy, r => 3.5, fill => $s->[3]);
        $b .= text(x => 30, y => $sy + 4, size => 12, color => $C{text}, t => $s->[0]);
        $b .= text(x => 426, y => $sy + 4, size => 10, color => $s->[3], anchor => 'end', t => $s->[2]);
    ...
    $b .= text(x => 14, y => 414, size => 9.5, color => $C{faint}, t => "Strips come from GitHub's public incident feed, not from published uptime figures.");
    $b .= footer($w, $h);
